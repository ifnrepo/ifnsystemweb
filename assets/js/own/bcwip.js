...
$(document).ready(function () {
	...
	table = $('#tabelnya').DataTable({
		...
		"ajax":
		{
			"url": base_url +"bcwip/getdatabaru/1", // URL file untuk proses select datanya
			"type": "POST",
			"data": function(d){
		...
		"columns": [
			...
			{ "data": "inkgs",
				...
				"render": function(data, type, row, meta){
					// return rupiah(data,'.',',',2);
					var xadjpcs = row.adjpcs > 0 ? parseFloat(row.adjpcs) : 0;
					var xadjkgs = row.adjkgs > 0 ? parseFloat(row.adjkgs) : 0;
					var pcs = rupiah(parseFloat(row.inpcs)+xadjpcs,'.',',',0);
					var kgs = rupiah(parseFloat(row.inkgs)+xadjkgs,'.',',',2);
					return "<span class='text-teal font-11'>"+pcs+"</span>"+"<br><span>"+kgs+"</span>";
				}
			},
			{ "data": "outkgs",
				...
				"render": function(data, type, row, meta){
					// return rupiah(data,'.',',',2);
					var xadjpcs = row.adjpcs < 0 ? parseFloat(row.adjpcs)*-1 : 0;
					var xadjkgs = row.adjkgs < 0 ? parseFloat(row.adjkgs)*-1 : 0;
					var pcs = rupiah(parseFloat(row.outpcs)+xadjpcs,'.',',',0);
					var kgs = rupiah(parseFloat(row.outkgs)+xadjkgs,'.',',',2);
					return "<span class='text-teal font-11'>"+pcs+"</span>"+"<br><span>"+kgs+"</span>";
				}
			},
			{ "data": "adjkgs",
