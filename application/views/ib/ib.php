        ...
        <div id="sisipkan" class="sticky-top bg-white">
          <div class="row mb-1 d-flex align-items-between">
            <div class="col-sm-6 mb-1">
              <?php $disab = '';
              if ($this->session->userdata('depttuju') == '' || $this->session->userdata('depttuju') == null) {
                $disab = 'disabled';
              } ?>
              <?php $cekpakaibc = datauser($this->session->userdata('id'), 'cekpakaibc') == 1 ? '' : 'hilang';
              $tmb = $this->session->userdata('depttuju') == 'FG' ? '/1' : ''; ?>
                    ...
                    <div class="font-kecil">
                      <select class="form-select form-control form-sm font-kecil font-bold" id="dept_tuju" name="dept_tuju">
                        <?php foreach ($hakdep->result_array() as $dep) { ?>
                          <option value="<?= $dep['dept_id']; ?>" <?php if ($this->session->userdata('depttuju') == $dep['dept_id']) {
                                                                    echo "selected";
            ...
            <tbody class="table-tbody" id="body-table" style="font-size: 13px !important;">
              ...
              ?>
                <tr>
                  ...
                  <td class='font-bold'><a href="<?= base_url() . 'ib/viewdetail/' . $datdet['id'] . $tmb; ?>" data-bs-toggle="offcanvas" data-bs-target="#canvasdet" data-title="View detail IB (AJU Masuk Barang)"><?= $datdet['nomor_dok'] ?></a></td>
                  <td><?= $namasup ?></td>
                  <td><?= $jmlrek ?></td>
                  ...
                  <td class="text-right line-12"><span style="color: white;">.</span>
                    ...
                    ?>
                      ...
                      <a href="#" data-href="<?= base_url() . 'ib/hapusaju/' . $datdet['id'] . $tmb ?>" class='btn btn-sm btn-danger <?= $butsudahkirim ?>' data-bs-toggle="modal" data-bs-target="#modal-danger" data-message="Hapus AJU <br><?= $datdet['nomor_dok']; ?>" title='Hapus data Transaksi'><i class="fa fa-trash"></i></a>
