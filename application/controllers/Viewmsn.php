...
class Viewmsn extends CI_Controller {
	...
	{  
        ...
        $data['data'] = $this->mastermsnmodel->getdatabyid($this->session->userdata('ceknomesin'))->row_array();
        $data['lokasi'] = $this->mastermsnmodel->getdatalokasi();
        $footer['fungsi'] = 'datamesin';
		// $this->load->view('layouts/header',$header);
		$this->load->view('mastermsn/viewmsn',$data);
		// $this->load->view('layouts/footer',$footer);
	}
    public function mesinno($id){
        $isinya = decrypto($id);
        $this->session->set_userdata('ceknomesin',$isinya);
        $url = base_url().'viewmsn';
        redirect($url);
    }
    ...
    public function excel(){
        ...
        $sheet->setCellValue('C2', "NAMA SATUAN"); // Set kolom C3 dengan tulisan "NAMA SATUAN"      
        ...
        foreach($satuan->result_array() as $data){ 
            ...
            $sheet->setCellValue('C'.$numrow, $data['namasatuan']);                
            $no++; 
    ...
    public function cetakpdf(){
        ...
        $pdf->Cell(140,8,'Nama Satuan',1,0,'C');
        $pdf->SetFont('Lato', '', 10);
        $pdf->ln(8);
        ...
        foreach ($detail->result_array() as $det) {
            ...
            $pdf->Cell(140,6,$det['namasatuan'],1);
            $pdf->ln(6);
        }
        $pdf->SetFont('Lato','',8);
        $pdf->ln(10);
