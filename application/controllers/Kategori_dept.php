...
class Kategori_dept extends CI_Controller
{
    ...
    public function excel()
    {
        ...
        $sheet->setCellValue('A1', "DATA KATEGORI DEPARTEMEN"); // Set kolom A1 dengan tulisan "DATA SISWA"    
        $sheet->getStyle('A1')->getFont()->setBold(true); // Set bold kolom A1    

        // Buat header tabel nya pada baris ke 3    
        $sheet->setCellValue('A2', "NO"); // Set kolom A3 dengan tulisan "NO"    
        $sheet->setCellValue('B2', "NAMA KATEGORI"); // Set kolom B3 dengan tulisan "KODE"    
        // Panggil model Get Data   
        $kategori = $this->kategori_dept_model->getdata();
        $no = 1;
        ...
        $sheet->setTitle("Data Kategori Departemen");

        // Proses file excel    
        header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
        header('Content-Disposition: attachment; filename="Data Kategori Departmen.xlsx"'); // Set nama file excel nya    
        header('Cache-Control: max-age=0');
        $writer = new Xlsx($spreadsheet);
        $writer->save('php://output');
        $this->helpermodel->isilog('Download Excel DATA KATEGORI DEPARTEMEN ');
    }
    public function cetakpdf()
    {
        ...
        $pdf->Cell(30, 18, 'DATA KATEGORI DEPARTEMEN');
        $pdf->ln(12);
        $pdf->SetFont('Latob', '', 10);
        $pdf->Cell(15, 8, 'No', 1, 0, 'C');
        $pdf->Cell(105, 8, 'Nama Kategori', 1, 0, 'C');

        $pdf->SetFont('Lato', '', 10);
        $pdf->ln(8);
        $detail = $this->kategori_dept_model->getdata();
        $no = 1;
        foreach ($detail as $det) {
            $pdf->Cell(15, 6, $no++, 1, 0, 'C');
            $pdf->Cell(105, 6, $det['nama'], 1);
        ...
        $pdf->Output('I', 'Data Kategori Departemen.pdf');
        $this->helpermodel->isilog('Download PDF DATA KATEGORI DEPARTEMEN');
    }
}
