<?php
class Billmaterialmodel extends CI_Model
{
    ...
    public function getdatabyid($id)
    {
        ...
        $this->db->from('ref_bom');
        $this->db->join('barang','barang.id = ref_bom.id_barang','left');
        $this->db->where('ref_bom.id',$id);
        return $this->db->get()->row_array();
    }
    public function getdatadetailbyid($id)
    {
        $this->db->select('ref_bom_detail.*,barang.nama_barang');
        $this->db->from('ref_bom_detail');
        $this->db->join('barang','barang.id = ref_bom_detail.id_barang','left');
    ...
    public function hapusdetail($id,$head){
        $this->db->trans_start();
        $this->db->where('id',$id);
        $hapus = $this->db->delete('ref_bom_detail');
        ...
        if($hasil['persen'] != 100.00){
            ...
            $this->db->update('ref_bom',['data_ok' => 0]);
        }else{
            $this->db->where('id',$head);
            $this->db->update('ref_bom',['data_ok' => 1]);
        }
        $this->helpermodel->isilog($this->db->last_query());
    ...
    {
        $this->db->trans_start();
        $this->db->insert('ref_bom_detail',$data);
        $this->helpermodel->isilog($this->db->last_query());
    ...
    public function updatedetail($data)
    {
        ...
        $this->db->update('ref_bom_detail',$data);
        $this->helpermodel->isilog($this->db->last_query());
    ...
    public function simpandata($data)
    {
        ...
        $cekdata = $this->db->get_where('ref_bom',$datakondisi);
