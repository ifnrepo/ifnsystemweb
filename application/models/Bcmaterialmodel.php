...
class bcmaterialmodel extends CI_Model
{
    ...
    public function getdata($mode=0){ // jika 0 mode Inventory, jika 1 Mode IT Inventory
        ...
        $kolom .= "(select sum(kgs) from stokopname_detail where trim(po)=trim(r1.po) and trim(item)=trim(r1.item) and dis=r1.dis and id_barang = r1.id_barang and dept_id = 'GM' and tgl='".tglmysql($tglakhir)."') as kgs_taking,";
        $kolom .= "(select sum(pcs) from stokopname_detail where trim(po)=trim(r1.po) and trim(item)=trim(r1.item) and dis=r1.dis and id_barang = r1.id_barang and dept_id = 'GM' and tgl='".tglmysql($tglakhir)."') as pcs_taking ";
    ...
    public function getopname(){
        ...
        $cek = $this->db->get_where('stokopname_detail',['tgl' => tglmysql($tglakhir),'dept_id' => "GM"],1)->num_rows();
        if($cek > 0){
            return $this->db->get_where('stokopname_detail',['tgl' => tglmysql($tglakhir),'dept_id' => "GM"],1)->row_array();
        }else{
            return ['tgl' => ''];
        }
    }
