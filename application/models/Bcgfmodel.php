...
class bcgfmodel extends CI_Model
{
    ...
    public function getdata($mode=0){ // jika 0 mode Inventory, jika 1 Mode IT Inventory
        ...
        $kolom .= "(select sum(kgs) from stokopname_detail where trim(po)=trim(r1.po) and trim(item)=trim(r1.item) and dis=r1.dis and id_barang = r1.id_barang and nobale = r1.nobale and dept_id = 'GF' and tgl='".tglmysql($tglakhir)."') as kgs_taking,";
        $kolom .= "(select sum(pcs) from stokopname_detail where trim(po)=trim(r1.po) and trim(item)=trim(r1.item) and dis=r1.dis and id_barang = r1.id_barang and nobale = r1.nobale and dept_id = 'GF' and tgl='".tglmysql($tglakhir)."') as pcs_taking ";
        ...
        $kolom .= " group by po,item,dis,nobale order by po,item,dis,nobale) r2";
        $hasil = $this->db->query($kolom);
