...
class bcwipmodel extends CI_Model
{
    ...
    public function getdata($mode=0){ // jika 0 mode Inventory, jika 1 Mode IT Inventory
        ...
        $this->db->select('stokdept.dept_id as deptt');
        ...
        $this->db->select('0 as totalpcsout,0 as totalkgsout');
        $this->db->select('0 as totalpcsadj,0 as totalkgsadj');
        $this->db->select('tb_header.dept_tuju as deptt');
        ...
        $this->db->join('tb_header','tb_header.id = tb_detail.id_header','left');
        $this->db->where_in('dept_tuju',$arrdept);
        ...
        $this->db->select('0 as totalpcsadj,0 as totalkgsadj');
        $this->db->select('tb_header.dept_id as deptt');
        ...
        $kolom = " Select *,sum(saldokgs+inkgs-outkgs+adjkgs) over() as totalkgs,sum(saldopcs+inpcs-outpcs+adjpcs) over() as totalpcs,sum(saldopcs) over() as sawalpcs,sum(saldokgs) over() as sawalkgs,sum(inpcs) over() as totalinpcs,sum(outpcs) over() as totaloutpcs,sum(inkgs) over() as totalinkgs,sum(outkgs) over() as totaloutkgs,sum(adjpcs) over() as totaladjpcs,sum(adjkgs) over() as totaladjkgs,ifnull(kgs_taking,0) as kgstaking,ifnull(pcs_taking,0) as pcstaking,sum(kgs_taking) over() as totalkgstaking,sum(pcs_taking) over() as totalpcstaking from (Select kategori.jns,kodeinv,nobale,po,item,dis,id_barang,xdln,left(concat(ifnull(id_kategori_po,''),ifnull(barang.id_kategori,'')),4) as id_kategori,trim(xnomor_bc) as nomor_bc,insno,nobontr,barang.kode,idu,stok,exnet,sum(saldopcs) as saldopcs,sum(saldokgs) as saldokgs,sum(inpcs) as inpcs,sum(inkgs) as inkgs,sum(outpcs) as outpcs,sum(outkgs) as outkgs,sum(adjpcs) as adjpcs,sum(adjkgs) as adjkgs,(sum(saldopcs)+sum(inpcs)-sum(outpcs)+sum(adjpcs)) as sumpcs,(sum(saldokgs)+sum(inkgs)-sum(outkgs)+sum(adjkgs)) as sumkgs,satuan.kodesatuan,barang.nama_barang,barang.imdo,spek,exdo,id_buyer,deptt,skupo,";
        $kolom .= "(select sum(kgs) from stokopname_detail where trim(po)=trim(r1.po) and trim(item)=trim(r1.item) and dis=r1.dis and id_barang = r1.id_barang and dept_id = r1.deptt and tgl='".tglmysql($tglakhir)."') as kgs_taking,";
        $kolom .= "(select sum(pcs) from stokopname_detail where trim(po)=trim(r1.po) and trim(item)=trim(r1.item) and dis=r1.dis and id_barang = r1.id_barang and dept_id = r1.deptt and tgl='".tglmysql($tglakhir)."') as pcs_taking ";
    ...
    public function getdatadetailbaru($array){
        ...
        // Query untuk CekSaldobarang
        $this->db->select("0 as kodeinv,'SALDO' as mode,'".tglmysql($this->session->userdata('tglawalbcwip'))."' as tgl,stokdept.po,stokdept.item,stokdept.dis,stokdept.id_barang,stokdept.dln as xdln,barang.id_kategori,stokdept.insno,stokdept.nobontr,barang.kode,stokdept.id as idu");
        ...
        // Query untuk In Barang
        $this->db->select("1 as kodeinv,'IN' as mode,tb_header.tgl,tb_detail.po,tb_detail.item,tb_detail.dis,tb_detail.id_barang,tb_detail.dln as xdln,barang.id_kategori,tb_detail.insno,tb_detail.nobontr,barang.kode,0 as idu");
        ...
        $this->db->group_by('po,item,dis,id_barang,nobontr,nomor_dok');
        $query2 = $this->db->get_compiled_select();

        // Query untuk Out Barang
        $this->db->select("2 as kodeinv,'OUT' as mode,tb_header.tgl,tb_detailgen.po,tb_detailgen.item,tb_detailgen.dis,tb_detailgen.id_barang,tb_detailgen.dln as xdln,barang.id_kategori,tb_detailgen.insno,tb_detailgen.nobontr,barang.kode,0 as idu");
        $this->db->select("kategori.nama_kategori,barang.safety_stock,tb_header.nomor_dok,satuan.kodesatuan");
        $this->db->select('0 as saldopcs,0 as saldokgs,tb_hargamaterial.jns_bc,tb_hargamaterial.tgl_bc');
        $this->db->select('0 as inpcs,0 as inkgs');
        $this->db->select('sum(pcs) as outpcs,sum(kgs) as outkgs');
        ...
        // Query untuk ADJ Barang
        $this->db->select("3 as kodeinv,'ADJ' as mode,tb_header.tgl,tb_detail.po,tb_detail.item,tb_detail.dis,tb_detail.id_barang,tb_detail.dln as xdln,barang.id_kategori,tb_detail.insno,tb_detail.nobontr,barang.kode,0 as idu");
        $this->db->select("kategori.nama_kategori,barang.safety_stock,tb_header.nomor_dok,satuan.kodesatuan");
        $this->db->select('0 as saldopcs,0 as saldokgs,tb_hargamaterial.jns_bc,tb_hargamaterial.tgl_bc');
        $this->db->select('0 as inpcs,0 as inkgs');
        $this->db->select('0 as outpcs,0 as outkgs');
        ...
        $this->db->group_by('po,item,dis,id_barang,nobontr');
        $query4 = $this->db->get_compiled_select();
    ...
    {
        if ($this->session->userdata('tglawalbcwip') == null) {
            $tglawal = tglmysql(date('Y-m-01'));
            $tglakhir = tglmysql(lastday($this->session->userdata('th') . '-' . $this->session->userdata('bl') . '-01'));
